...
class oradb_11g {
  require oradb_os
    ...
    oradb::rcu{  'DEV_RCU':
      ...
      product                   => lookup('repository_type'),
      version                   => '11.1.1.9',
      user                      => lookup('oracle_os_user'),
      group                     => lookup('oracle_os_group'),
      download_dir              => lookup('oracle_download_dir'),
      action                    => 'create',
      oracle_home               => lookup('oracle_home_dir'),
      db_server                 => lookup('oracle_database_host'),
      db_service                => lookup('oracle_database_service_name'),
      sys_password              => lookup('oracle_database_sys_password'),
      schema_prefix             => lookup('repository_prefix'),
      repos_password            => lookup('repository_password'),
